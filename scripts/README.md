...
`seed_dast_user.py` and the `check_*` scripts are run ad hoc (by implementation/CI,
respectively) against the working tree. `check_simulator_storage.sh` is Mac-only and
operator-facing — it reads no repo source at all, only the host's disk, and is the one
script here whose failure means "stop before starting a run" rather than "fix the code"
(see `docs/simulator-storage.md`). Everything under `ci/` is the same script CI
